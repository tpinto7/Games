<template>
    <b-modal id="cartModal" size="lg" no-fade ref="cartModal" cancel-variant="outline-secondary" @keydown.enter.native.prevent="saveForm">
        ...
        Total Cost: {{cost}}
        <div v-if="ordered"> <b-table striped hover :items="shelfs"></b-table> </div> 
...
<script>
        ...
        methods: {
            async calculateIngredients(){
                ...
                for(let i = 0; i < product.length; i ++){
                   let cost = product[i].price.substring(1)
                    let sku;
                ...
                if(check){
                    this.items.push({sku: sku, 
                            name: name, required: product[i].usedAmnt.split(" ")[0] * this.orders[j].servings , value: value, unit: unit, amount: 0, cost: cost})
                    }
                }
                }
                this.subtractPantry();
            ...
            getLocations(){
                for(let i = 0; i < this.items.length; ++i ){
                    if(this.items[i].amount > 0){
                        let url = 'https://api.wegmans.io/products/' + this.items[i].sku + '/locations/68?api-version=2018-10-18&subscription-key=' + this.subKey;
                        this.$axios.get(url).then((response) => {
                            let product = response.data;
                            if(response.data.locations && response.data.locations.length > 0){
                                this.shelfs.push({name: this.items[i].name, 
                                        location: product.locations[0].name, 
                                        aisle: product.locations[0].aisleSide,
                                        shelf: product.locations[0].shelfNumber})
                            }
                        })
                    }
                }
            ...
            subtractPantry(){
                for(let i = 0; i < this.items.length; ++i ){
                    ...
                    this.items[i].cost *= this.items[i].amount; 
                    if(!isNaN(this.items[i].cost)) { this.cost += this.items[i].cost; }
                }
                this.loading = true;
            ...
            async saveForm(){
                // for(let i = 0; i < this.items.length; ++i){
                //     if(!this.items[i].name.charAt(0).match(/^\d/)){
                // let url = 'http://127.0.0.1:8000/api/' + this.$root.userStore.state.id + '/pantry?sku=' +
                //     this.items[i].sku + '&value=' + this.items[i].value + '&unit=' + this.items[i].unit + 
  ...
  type: 'success',
  title: 'Order Placed',
  text: 'Your pantry has been updated!'
});
            }
        }
    };
